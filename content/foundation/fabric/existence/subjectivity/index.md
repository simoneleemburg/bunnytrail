Subjectivity is one of the three structural axes of
[[existence/mundus|Mundus]]. It is the measure of a thing's
closeness to [[existence/self|the Self]] — the horizon at which
reality is wholly the experiencing subject and nothing holds
apart from them.

A thing's Subjectivity is the degree to which the world it
inhabits is shaped by its experience of that world. Where
[[existence/essence|Essence]] measures the ratio of pattern
to particulars and [[existence/dissolution|Dissolution]]
measures closeness to coming-apart, Subjectivity measures the
_give_ of reality under the perceiver: how far the world bends
around the one looking at it, rather than holding its shape
regardless.

## What it looks like, walked along

Near the centre of Mundus, Subjectivity is low — ordinary, in
the way most things in Mundus are ordinary. Time runs at one
rate. Space is shared. Two observers in the same room agree on
the room. The perceiver is one of many in a world that holds
roughly the same shape for all of them.

Leaning further along the axis, reality begins to bend.
The flow of time and the lay of space are felt more intently;
they answer, slightly, to mood and attention. Colours shift
under feeling. Light has weight on a day of weight. The
perceiver and the perceived are no longer entirely separate.

Further still, the world's sensory texture tracks inner state
directly. Two people standing in what they call the same place
may be in places that share little but their name. What is
present is what is held; what is forgotten is, in some real
sense, not there.

Closer to the horizon, dimensions begin to fold around personal
experience. Spatial and temporal distance compress into
experiential adjacency: what matters is near, what does not
matter is far, and the ordinary measurements no longer carry.
Ages may be a doorway apart. A century may be a hillock.
Geography becomes a shape made by the one who walks it.

At the horizon itself — never reached — there is no shared
world left at all; there is only the single continuous interior
of one experiencing self.

Moving in the other direction, away from the Self, has its own
character. A little below the ordinary middle, a thing's hold
on being a subject begins to loosen. People describe this kind
of leaning as out-of-body experience, as perceiving long
stretches of time as though they were all happening now, as
seeing oneself in the third person. The world does not bend
less around the perceiver here so much as the perceiver
themselves becomes less of a fixed point from which the world
could bend. Further along, the category of the self loses its
hold entirely: there is still something there in Mundus, but
nothing it would make sense to call a subject, nothing for the
world to be a world _for_.

## Not an independent dimension

Subjectivity is not a quantity a thing has independently of its
Essence or Dissolution. A thing in Mundus has one position in
the interior; [[existence/essence|Essence]], Subjectivity,
and [[existence/dissolution|Dissolution]] are three readings of
that one position. They sum to a constant. A thing whose
Subjectivity is rising is, by the same fact, having its
Essence or Dissolution (or both) lowered.

The geometric consequence is that as reality starts to bend
around a thing, the thing moves across the interior, away from
at least one of the other two corners. A perceiver whose world
has begun to answer to them is, by that fact, no longer at
exactly the position they occupied before, and the position
they have left is some combination of pattern-participation
and closeness-to-coming-apart.

## How clusters speak of it

Where a cluster's thought speaks of perception shaping reality,
of inner state colouring the world, of memory as geography, of
the world bending around a subject — it is often speaking of
Subjectivity or of some specific manifestation of it. Local
vocabularies sit closer to particular regions of the axis than
to its full span: a cluster's account of, say, perceptual
bending under emotion is naming a stretch along the lower part
of the axis; a cluster's account of memory becoming traversable
landscape is naming the further reaches.

In Aurethia the phenomenon of motion along this axis is named
[[fabric/remembrance|Remembrance]]; the region of Mundus that
leans hardest toward the Self is known as
[[realms/valdor|Valdor]], where memory and ego become
geography.

## A condition, not a power

Subjectivity is not a faculty a thing exercises. It is not a
measure of how much a being _makes_ the world answer to them;
it is a measure of how much it _does_ answer, by whatever
route. A practitioner walking deliberately toward the Self and
a person caught in grief that folds time around them are both
leaning along the same axis, and to a similar degree, even
though one chose the leaning and the other did not.

Whether a being can _steer_ what bends around them — whether
they can shape the world's answering rather than only undergo
it — is a separate question from where they sit on the axis.
Cluster-specific practices exist along the axis at every
depth: some are about deliberate approach, some are about
holding ground against it, some are about returning from it
intact. The axis itself measures only the give, not the hand.

## What it does not mean

Subjectivity does not measure how _opinionated_ a thing is, or
how strongly it feels, or how much it remembers. The work it
measures is the work of reality answering to the perceiver, not
the work the perceiver does. A being can be vivid and
passionate and remain low in Subjectivity, if the world it
inhabits holds its shape regardless. A being can be quiet and
seemingly unremarkable and be high in Subjectivity, if the
world around it has begun, quietly, to bend.

The word **subjectivity** in ordinary use, including in this
compendium when uncapitalised, means the ordinary philosophical
notion of perspective-dependence. Only the capitalised
**Subjectivity** names the structural axis.
